...
import com.dipdev.autocaptioner.engine.CaptionAnimator.TimedWord
import com.dipdev.autocaptioner.engine.CaptionAnimator.WordTransform
...
object CaptionRenderer {
    ...
    private fun drawWord(
        ...
        canvas.scale(xfm.scaleX, xfm.scaleY)
        canvas.translate(-cx, -cy)
        canvas.translate(xfm.translateX, xfm.translateY)
        ...
        if (isBgPass) {
            if (style.outlineWidth > 0f) {
                ...
                canvas.drawText(drawTxt, x, y, CaptionPaints.outline)
            } else if (style.shadowRadius > 0f) {
                CaptionPaints.text.color = fillColor
                CaptionPaints.text.alpha = a
                canvas.drawText(drawTxt, x, y, CaptionPaints.text)
            }
        } else {
            CaptionPaints.text.color = fillColor
            CaptionPaints.text.alpha = a
            ...
            when (style.karaokeHighlightMode) {
                ...
                KaraokeHighlightMode.COLOR_CHANGE -> {
                    if (style.displayMode == DisplayMode.KARAOKE_FILL ||
                        ...
                        canvas.save()
                        canvas.clipRect(x, lineTop, x + wordW * fillP, lineBot)
                        CaptionPaints.highlight.alpha = (255 * xfm.alpha).toInt()
                        canvas.drawText(txt, x, y, CaptionPaints.highlight)
